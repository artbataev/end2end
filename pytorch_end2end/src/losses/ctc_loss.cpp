...
void CTCLossEngine::compute_2d(
    ...
    int targets_len, int batch_i, torch::Tensor& losses, torch::Tensor& grads) {
  ...
  auto prob_sum = torch::full(
      {logits_2d_a.size(0), logits_2d_a.size(1)}, -INFINITY,
...
std::tuple<at::Tensor, at::Tensor> CTCLossEngine::compute(
    const at::Tensor& logits_, const at::Tensor& targets_,
    const at::Tensor& logits_lengths_, const at::Tensor& targets_lengths_) {
...

  {
    ...
    for (int i = 0; i < batch_size; i++) {
      ...
      pool.add_task([this, &logits, &targets_a, i, seq_len, targets_len,
                     &losses, &grads] {
        compute_2d(logits[i], targets_a[i], seq_len, targets_len, i, losses,
                   grads);
      });
    }
  }
